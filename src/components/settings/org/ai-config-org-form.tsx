...
import {
  useOrgAiConfig,
  useUpsertOrgAiConfig,
  useDeleteOrgAiConfig,
} from "@/hooks";
...
export function AIConfigOrgForm({ slug }: { slug: string }) {
  const { data: aiConfig, isLoading } = useOrgAiConfig(slug);
  const upsertConfig = useUpsertOrgAiConfig(slug);
  const deleteConfig = useDeleteOrgAiConfig(slug);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      setModel(aiConfig.model as AIModel);
  ...
  return (
    <div className="space-y-6">
      {/* Info banner — explains cascading */}
      <div
        className={cn(
      ...
      >
        ...
        <div>
          <p className="text-sm font-medium text-foreground">
            Organisation-level AI config
          </p>
          <p className="text-sm text-muted-foreground mt-0.5">
            This config applies to all products and projects in your
            organisation unless they have their own AI config set. Project-level
            configs always take priority over this one.
          </p>
        </div>
      </div>
      ...
      {!aiConfig ? (
        ...
        >
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Configure an API key below to enable AI insights across your
              entire organisation.
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              {PROVIDER_LABELS[aiConfig.provider as AIProvider]} —{" "}
              {aiConfig.model} · key ending in{" "}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Default schedule for automated insight generation across all
            projects.
          </CardDescription>
        </CardHeader>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This will disable AI features for all projects that rely on
                  the organisation config. Projects with their own AI config
